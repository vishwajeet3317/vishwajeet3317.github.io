...
for (const category in emojisByCategory) {
  ...
  displayEmojis(
    category,
    categoryEmojis.map((data) => data.emoji)
  );
}
...
function addEmojiToTextArea(emoji) {
  ...
  clipboardPopup.value += emoji;
}
clipboardButton.addEventListener(
  "click",
  async () => await copyMultipleItemsToClipboard()
);
async function copyMultipleItemsToClipboard() {
  try {
    const clipboardItems = [];
    const textData = new Blob([clipboardPopup.value], { type: "text/plain" });
    const textItem = new ClipboardItem({ "text/plain": textData });
    clipboardItems.push(textItem);
    await navigator.clipboard.write(clipboardItems);
    clipboardPopup.value = "";
